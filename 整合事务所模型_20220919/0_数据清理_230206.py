...
# 保存文件夹名称
root_name = '会所专题底层数据_230407'
root_path = r"/Users/zhuangyuhao/PycharmProjects/AC_model/整合事务所模型_20220919"
url_pacpath = '/'+root_name+'/'


def read_df(file_name):
    df = pd.read_excel(root_path+'/会所专题底层数据_230407/' + file_name).dropna(axis=0,how='all')
...
def save_df(df_name):
    #  此处名称后续可修改
    exec("{}.to_csv(root_path + '会所专题底层数据_230407/{}.txt', index=False, sep=',')".format(df_name, df_name))
...
# 读取文件夹内所有表名
file_name_array = [f for f in os.listdir(root_path+'/会所专题底层数据_230407/') if f.endswith('.xlsx')]
...
for file_name in file_name_array:
    ...
    if file_name == "【WIND】上市公司分年份数据":
        ...
        df['AF_CYD_factor08'] = np.where(df['AF_CYD_factor08'] != df['AF_CYD_factor04'], np.nan, df['AF_CYD_factor08'])
        # df = df[~df['AF_CYD_factor02'].str.contains('\(退市\)')]
